...
void VS()
{
    ...
    vIPos = (iPos*vec4(1.0,0.2,1.0,1.0))+vec4(0.0,-cElapsedTime*10.0,0.0,0.0);//usetime
...
void PS()
{
    float colormix = fbm(vIPos.xyz*vec3(1.5,0.5,0.1));
    vec3 c = mix(vec3(0.0,0.3,0.3),vec3(0.2,0.3,0.0),bias(colormix,0.4));
    float hotedge = bias(dot(vNormal,normalize(cCameraPosPS-vWorldPos.xyz)),0.7);
